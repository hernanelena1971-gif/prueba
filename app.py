...
def logo_con_alto(path, alto_px):
    ...
    try:
        resample = PILImage.Resampling.LANCZOS  # Pillow >= 10
    except AttributeError:
        resample = PILImage.LANCZOS  # Pillow < 10

    return img.resize((nuevo_ancho, alto_px), resample)
...
if st.session_state.session is None:
    ...
    def logo_con_alto(path, alto_px):
        ...
        w, h = img.size
        nuevo_ancho = int((alto_px / h) * w)
        return img.resize((nuevo_ancho, alto_px), PILImage.LANCZOS)
...
mostrar_tabla("⚠️ Sales y otros parámetros", [
    ...
    ["Boro soluble en el extracto de saturación  (p.p.m.)", row["boro"]],
])
...
def generar_pdf_informe(row, codigo_sitio):
    ...
    tabla_pdf("Sales y otros parámetros", [
        ["Sodio intercambiable (meq/100 g)", row["sodio"]],
        ["Cloruro soluble en el extracto de saturación (meq/l)", row["cloruro_extracto"]],
        ["Cloruro sol. extr. de saturación - suelo seco (p.p.m.)", row["cloruro_suelo_seco"]],
        ["Estabilidad de Agregados de Suelo (%)", row["eas"]],
        ["Boro soluble en el extracto de saturación  (p.p.m.)", row["boro"]],
    ])
# --------------------------------------------------
